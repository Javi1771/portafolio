...
export default function ThemeToggle({ className = "" }) {
  ...
  return (
    <label className={`inline-flex items-center relative cursor-pointer group ${className}`}>
      ...
      <div
        className={`absolute -inset-2 md:-inset-4 rounded-full blur-xl md:blur-2xl transition-all duration-700 opacity-0 group-hover:opacity-100 ${
          isDark
      ...
      {/* Track principal */}
      <div
        className={`relative w-14 h-7 md:w-24 md:h-12 rounded-full transition-all duration-700 transform group-hover:scale-105 ${
          isDark
      ...
      >
        ...
        {/* Botón deslizante */}
        <div
          className={`absolute w-5 h-5 md:w-10 md:h-10 rounded-full top-1 transition-all duration-700 ease-out transform group-hover:scale-110 ${
            isDark ? "translate-x-7 md:translate-x-12" : "translate-x-1"
          }`}
        >
          {/* Aura del botón */}
          <div
            className={`absolute -inset-0.5 md:-inset-1 rounded-full blur-md md:blur-lg transition-all duration-700 ${
              isDark
          ...
          {/* Cuerpo principal */}
          <div
            className={`relative w-5 h-5 md:w-10 md:h-10 rounded-full transition-all duration-700 ${
              isDark
          ...
          >
            {/* Reflejo */}
            <div className="absolute top-0.5 left-0.5 right-0.5 h-2 md:top-1 md:left-1 md:right-1 md:h-4 bg-gradient-to-b from-white/90 via-white/60 to-transparent rounded-full blur-sm" />
            ...
            >
              {isDark ? (
                <div className="relative">
                  <svg className="w-3 h-3 md:w-5 md:h-5 fill-slate-600 drop-shadow-lg" viewBox="0 0 24 24">
                    <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
                  </svg>
                  <div className="absolute top-0.5 right-0.5 md:top-1 md:right-1 w-0.5 h-0.5 md:w-1 md:h-1 bg-slate-400 rounded-full opacity-60" />
                  <div className="absolute bottom-1 left-1 md:bottom-1.5 md:left-1.5 w-px h-px md:w-0.5 md:h-0.5 bg-slate-500 rounded-full opacity-40" />
                  <div className="absolute -inset-1 md:-inset-2 bg-violet-400/20 rounded-full blur-md animate-pulse" />
                </div>
              ) : (
                <div className="relative">
                  <svg className="w-3 h-3 md:w-5 md:h-5 fill-amber-500 drop-shadow-lg" viewBox="0 0 24 24">
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-1 md:w-2 md:h-2 bg-yellow-300 rounded-full opacity-80" />
                  <div className="absolute -inset-1 md:-inset-2 bg-amber-400/30 rounded-full blur-md animate-pulse" />
                </div>
              )}
            </div>
            ...
            {/* Punto de luz */}
            <div
              className={`absolute w-1 h-1 md:w-2 md:h-2 rounded-full transition-all duration-700 ${
                isDark
                  ? "top-0.5 right-0.5 md:top-1.5 md:right-1.5 bg-gradient-to-br from-violet-300 to-indigo-400 shadow-[0_0_8px_rgba(139,92,246,0.8)]"
                  : "top-0.5 left-0.5 md:top-1.5 md:left-1.5 bg-gradient-to-br from-white to-amber-100 shadow-[0_0_8px_rgba(255,255,255,0.9)]"
              }`}
            />
          </div>
        </div>
